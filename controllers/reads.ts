...
export const getReads = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [count, reads] = await Promise.all([
            ReadsModel.countDocuments(),
            ReadsModel.find()
            .skip(Number(fromNumber))
            .limit(Number(limitNumber))
        ]);
        res.json({ msg: 'success', count, reads });
    } catch (error) {
...
export const updateRead = async (req: Request, res: Response) => {
    res.status(200).json({ msg: 'updateRead'});
    // try {
    //     const { id } = req.params;
    //     const body = req.body;
...
export const deleteRead = async (req: Request, res: Response) => {
    res.status(200).json({ msg: 'deleteRead'});
    // try {
    //     const { id } = req.params;
